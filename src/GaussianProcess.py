...
def get_data(N: int = 100, N_test: int = 200):
...
def main(N: int, num_warmup: int, num_samples: int, num_chains: int, post_samples: int):
    ...
    vmap_args = (random.split(jax.random.PRNGKey(324), post_samples),) + tuple(
        subsamples
    )
    mu, predictions, K1 = vmap(
    ...
    vmap_args = (random.split(jax.random.PRNGKey(675), post_samples),) + tuple(
        subsamples
    )
    log_scale, predictions, K2 = vmap(
        lambda rng_key, var, length, noise: predict(
            rng_key,
            X,
